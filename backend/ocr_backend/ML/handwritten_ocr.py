...
# /Users/adityagupta/Desktop/Coding/MosipBackend/ocr_extract/backend/ocr_backend/ML/model_cache/models--microsoft--trocr-large-handwritten/snapshots/e68501f437cd2587ae5d68ee457964cac824ddee
...
def load_file_as_numpy_image(file_path):
    ...
    # # If file is not PDF, try loading as image ALWAYS 
    try:
        return np.ascontiguousarray(np.array(Image.open(file_path).convert("RGB")))
    except:
        return None
...
def detect_text_craft(image_rgb):
    ...
    prediction_result = get_prediction(
        ...
        craft_net=craft_net,
        refine_net=refine_net,
        text_threshold=0.3,   # ↓ lower sensitivity
        link_threshold=0.1,   # ↓ improve grouping
        low_text=0.2,         # ↓ detect faint handwriting
        cuda=USE_GPU,
        long_size=1500        # ↑ larger image improves detection
    )

    # Debug raw bbox output
    ...
    # FIX — Work with numpy arrays safely
    ...
    for box in raw_boxes:
        ...
        if w > 5 and h > 5:  # ↓ reduce min size threshold
            formatted_boxes.append((x_min, y_min, w, h))
...
#     """
#     Main function to be used by Django API.
#     Returns: structured fields + text lines.
#     """
